...
type (
	...
	createGroupReq struct {
		...
		Type            Type              `json:"Type"`                      // （必填）群组形态，包括 Public（陌生人社交群），Private（即 Work，好友工作群），ChatRoom（即 Meeting，会议群），AVChatRoom（直播群），Community（社群）
		Name            string            `json:"Name"`                      // （必填）群名称，最长100字节，使用 UTF-8 编码，1个汉字占3个字节
		Introduction    string            `json:"Introduction,omitempty"`    // （选填）群简介，最长400字节，使用 UTF-8 编码，1个汉字占3个字节
		Notification    string            `json:"Notification,omitempty"`    // （选填）群公告，最长400字节，使用 UTF-8 编码，1个汉字占3个字节
		FaceUrl         string            `json:"FaceUrl,omitempty"`         // （选填）群头像 URL，最长500字节
